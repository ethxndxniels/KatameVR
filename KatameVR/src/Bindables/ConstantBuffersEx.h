	...
	namespace Bind
	{
		...
		public:
			void Update( Graphics* gfx, const Dcb::Buffer& buf )
			{
				...
				gfx->m_Context->Unmap( (ID3D11Resource*)pConstantBuffer, 0u );
			}
			void Bind( Graphics* gfx ) noexcept override
			{
				gfx->m_Context->PSSetConstantBuffers( slot, 1u, &pConstantBuffer );
			}
		...
		protected:
				...
				slot( slot )
			{
				...
				if (pBuf != nullptr)
				{
					...
					gfx->m_Device->CreateBuffer( &cbd, &csd, &pConstantBuffer );
				}
				else
				{
					gfx->m_Device->CreateBuffer( &cbd, nullptr, &pConstantBuffer );
				}
			}
		private:
